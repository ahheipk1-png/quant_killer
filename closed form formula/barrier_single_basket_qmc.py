"""Sobol-QMC benchmark for barrier_single_basket.py.

Simulates the ACTUAL correlated multi-asset basket path (fine grid, per-
asset term-vol curve, fixed correlation matrix) and monitors the true
basket value B_t = sum w_i S_i(t) against the barrier -- deliberately NOT
using the closed form's effective-lognormal shortcut, so the gap between
the two is exactly the moment-matching approximation error.
"""
...
from barrier_single_basket import total_variance, _OBS_PER_YEAR
...
def qmc_price_barrier_single_basket(
    spots,
    weights,
    strike,
    rate,
    ...
    *,
    ...
    w_grid = np.array([total_variance(vol_times_list[i], vol_values_list[i],
                                       np.linspace(dt, maturity, steps)) for i in range(n)])  # (n, steps)
    dW = np.diff(np.concatenate([np.zeros((n, 1)), w_grid], axis=1), axis=1)
    dW = np.maximum(dW, 0.0)
    sqrt_dW = np.sqrt(dW)
    drift = carries[:, None] * dt - 0.5 * dW  # (n, steps)
    ...
    for shift_index, z in enumerate(_shifted_sobol_normals(n_points, n * steps, n_shifts, seed)):
        ...
        z_corr = np.moveaxis(z @ chol.T, 2, 1)  # (n_points, n, steps)
        ...
        basket_path = np.tensordot(full, weights, axes=([1], [0]))  # (n_points, steps+1)

        if trigger == "european":
            terminal = basket_path[:, -1]
            hit = (terminal >= barrier) if direction == "up" else (terminal <= barrier)
            hit_idx = np.zeros(n_points, dtype=int)
        else:
            touched = (basket_path >= barrier) if direction == "up" else (basket_path <= barrier)
